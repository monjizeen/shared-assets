...
Primitives live in `components/ui/` (shadcn-vue). App patterns live in `components/`, `layouts/`, `composables/`.
...
1. **Source of truth for tokens/rules:** `monjizeen/.cursor/rules/design.mdc`
2. **Source of truth for page structure:** `monjizeen/.cursor/rules/frontend-conventions.mdc`
3. When a pattern stabilizes in monjizeen, update this doc + [INDEX.md](./INDEX.md)
4. Long-term: port high-value patterns into `shared-assets/templates/web-app` via `build-web-app-template.sh`
